...
impl SSTableManager {
    ...
    pub fn get(&self, key: &Vec<u8>) -> Option<Vec<u8>> {
        for (_, entry) in self.files.iter().rev() {
    ...
    pub fn flush(&mut self, entries: Vec<entry::Entry>) {
        ...
        self.sstable_num += 1;
        self.file_table.insert(self.sstable_max_id, Vec::new());
    ...
    pub fn update_cur_block_id(&mut self) {
        if self.cur_block_id == self.block_id + self.block_num as u32 {
